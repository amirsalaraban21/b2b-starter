...
export async function GET(req: AuthenticatedMedusaRequest, res: MedusaResponse) {
  ...
  return res.json({ application: { status: application.status, professional_type: application.professional_type, organization_name: application.organization_name, submitted_at: application.created_at, reviewed_at: application.reviewed_at, customer_feedback: application.status === "needs_information" ? application.customer_feedback : null } })
}
