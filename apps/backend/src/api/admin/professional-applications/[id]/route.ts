...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  const application = await service.retrieveProfessionalApplication(
    req.params.id
  );
...
export async function POST(
  req: AuthenticatedMedusaRequest<UpdateProfessionalApplicationType>,
  res: MedusaResponse
) {
  const service = req.scope.resolve<ProfessionalApplicationModuleService>(
    PROFESSIONAL_APPLICATION_MODULE
  ...
  const allowed: Record<string, string[]> = {
    ...
    needs_information: ["pending", "approved", "rejected"],
    approved: [],
    rejected: [],
  };
  if (!allowed[existing.status]?.includes(req.validatedBody.status))
    return res
      .status(409)
      .json({ message: "Invalid professional application status transition." });
  const application = await service.updateProfessionalApplications({
    id: existing.id,
    status: req.validatedBody.status,
    admin_notes: req.validatedBody.admin_notes?.trim() || null,
    customer_feedback:
      req.validatedBody.status === "needs_information"
        ? req.validatedBody.customer_feedback?.trim() || null
        : null,
