  ...
  return (
    <section className="content-container py-10 small:py-14">
      <div className="grid overflow-hidden border-y border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-900 small:grid-cols-[.9fr_1.1fr]">
        <div className="relative min-h-72 overflow-hidden p-7 small:min-h-[420px] small:p-10" style={{ backgroundColor: `${battery.color}14` }}>
          ...
          <div className="relative mx-auto h-64 max-w-md overflow-hidden small:h-80">
        ...
        <div className="flex flex-col justify-center p-7 small:p-10">
          <p className="text-xs font-bold text-teal-700">{fa ? "راهنمای سریع باتری" : "BATTERY SIZE GUIDE"}</p>
          <h2 className="mt-2 text-3xl font-black leading-tight">{fa ? "باتری مناسب سمعک خود را پیدا کنید" : "Find the right battery size"}</h2>
          ...
          <div className="mt-7 grid grid-cols-4 gap-2" role="tablist" aria-label={fa ? "انتخاب سایز باتری" : "Select battery size"}>
            {batteries.map((item, index) => (
              <button key={item.size} type="button" role="tab" aria-selected={active === index} onMouseEnter={() => setActive(index)} onFocus={() => setActive(index)} onClick={() => setActive(index)} className={`border bg-white px-2 py-4 text-center transition duration-200 motion-reduce:transition-none ${active === index ? "border-slate-900 shadow-sm dark:border-white" : "border-slate-200 hover:-translate-y-0.5 dark:border-slate-700"}`}>
