...
const categoryImages = "/products/earmed/care-assortment-v2.png"
  ...
  return <main dir={fa ? "rtl" : "ltr"} className="bg-white text-slate-950 dark:bg-slate-950 dark:text-white">
    <section className="border-b border-slate-200 bg-[#f3f6f5] dark:border-slate-800 dark:bg-slate-900">
      <div className="content-container grid min-h-[470px] items-center gap-8 py-10 small:grid-cols-[45%_55%]">
        ...
        <div className="relative h-[330px] overflow-hidden small:h-[430px]"><img src={categoryImages} alt={fa ? "باتری و لوازم مراقبت سمعک" : "Hearing aid batteries and care accessories"} className="h-full w-full object-cover" /></div>
      </div>
    </section>
    ...
    <section className="content-container py-10 small:py-14"><div className="mb-6 flex items-end justify-between"><div><p className="text-xs font-bold text-teal-700">{fa ? "دسته‌بندی‌ها" : "DEPARTMENTS"}</p><h2 className="mt-1 text-2xl font-black">{fa ? "سریع‌تر به محصول مورد نیاز برسید" : "Shop by department"}</h2></div><LocalizedClientLink href="/store" className="text-sm font-bold text-teal-700">{fa ? "همه محصولات" : "View all"}</LocalizedClientLink></div><div className="grid gap-4 xsmall:grid-cols-2 small:grid-cols-4">{categories.map(([title, text], index) => <LocalizedClientLink href="/store" key={title} className="group border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900"><div className="h-44 overflow-hidden bg-slate-50"><img src={categoryImages} alt="" className="h-full w-full object-cover transition group-hover:scale-105" style={{ objectPosition: `${20 + index * 20}% center` }} /></div><div className="p-4"><h3 className="font-bold">{title}</h3><p className="mt-1 text-xs leading-5 text-slate-500">{text}</p></div></LocalizedClientLink>)}</div></section>

    {region && products.length > 0 && <section className="border-y border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-900"><div className="content-container py-10 small:py-14"><div className="mb-6 flex items-center justify-between"><h2 className="text-2xl font-black">{fa ? "محصولات منتخب" : "Selected products"}</h2><LocalizedClientLink href="/store" className="text-sm font-bold text-teal-700">{fa ? "مشاهده فروشگاه" : "Open store"}</LocalizedClientLink></div><ul className="flex snap-x gap-4 overflow-x-auto pb-3">{products.slice(0, 5).map(product => <li key={product.id} className="w-[76vw] shrink-0 snap-start xsmall:w-[46%] small:w-[calc(25%-12px)]"><ProductPreview product={product} region={region} /></li>)}</ul></div></section>}
