...
export default async function StorefrontHome({ countryCode }: { countryCode: string }) {
  ...
  return <main dir={fa ? "rtl" : "ltr"} className="bg-white text-slate-950 dark:bg-slate-950 dark:text-white">
    ...
    <section className="content-container py-10 small:py-14"><div className="mb-6 flex flex-col items-start gap-3 xsmall:flex-row xsmall:items-end xsmall:justify-between"><div><p className="text-xs font-bold text-teal-700">{fa ? "دسته‌بندی‌ها" : "DEPARTMENTS"}</p><h2 className="mt-1 text-2xl font-black">{fa ? "سریع‌تر به محصول مورد نیاز برسید" : "Shop by department"}</h2></div><LocalizedClientLink href="/store" className="shrink-0 text-sm font-bold text-teal-700">{fa ? "همه محصولات" : "View all"}</LocalizedClientLink></div><div className="grid gap-4 xsmall:grid-cols-2 small:grid-cols-4">{categories.map(([title, text], index) => <LocalizedClientLink href="/store" key={title} className="group border border-slate-200 bg-white transition duration-200 motion-reduce:transform-none motion-reduce:transition-none hover:-translate-y-1 hover:border-teal-300 dark:border-slate-800 dark:bg-slate-900"><div className="h-44 overflow-hidden bg-slate-50"><img src={departmentReferenceImages[index]} alt="" className="h-full w-full object-cover transition-transform duration-300 motion-reduce:transform-none motion-reduce:transition-none group-hover:scale-[1.03]" /></div><div className="p-4"><h3 className="font-bold">{title}</h3><p className="mt-1 text-xs leading-5 text-slate-500">{text}</p></div></LocalizedClientLink>)}</div></section>
    ...
    {region && products.length > 0 && <section className="border-y border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-900"><div className="content-container py-10 small:py-14"><div className="mb-6 flex flex-col items-start gap-3 min-[360px]:flex-row min-[360px]:items-center min-[360px]:justify-between"><h2 className="text-2xl font-black">{fa ? "محصولات منتخب" : "Selected products"}</h2><LocalizedClientLink href="/store" className="shrink-0 text-sm font-bold text-teal-700">{fa ? "مشاهده فروشگاه" : "Open store"}</LocalizedClientLink></div><ul className="grid grid-cols-1 gap-4 min-[360px]:grid-cols-2 small:grid-cols-4 large:grid-cols-5">{products.slice(0, 5).map(product => <li key={product.id} className="min-w-0"><ProductPreview product={product} region={region} /></li>)}</ul></div></section>}

    <InteractiveBatteryFinder locale={locale} />

    <section className="content-container pb-12"><div className="grid overflow-hidden bg-slate-950 text-white small:grid-cols-[1.25fr_.75fr]"><div className="p-6 xsmall:p-8 small:p-12"><p className="text-xs font-bold text-teal-300">{fa ? "برای متخصصان و مراکز" : "FOR PROFESSIONALS"}</p><h2 className="mt-3 text-2xl font-black xsmall:text-3xl">{fa ? "خرید حرفه‌ای برای کلینیک‌ها و متخصصان" : "Professional purchasing for clinics"}</h2><p className="mt-4 max-w-xl leading-7 text-slate-300">{fa ? "درخواست همکاری حرفه‌ای خود را ثبت کنید و وضعیت آن را در حساب کاربری پیگیری کنید." : "Submit the professional application and follow its status from your account."}</p><LocalizedClientLink href="/professional" className="mt-7 inline-flex max-w-full bg-white px-5 py-3 text-center text-sm font-bold text-slate-950 xsmall:px-6">{fa ? "ثبت درخواست حرفه‌ای" : "Start professional application"}</LocalizedClientLink></div><div className="min-h-56 xsmall:min-h-64"><img src={departmentReferenceImages[2]} alt="" className="h-full w-full object-cover opacity-80" /></div></div></section>
  </main>
}
