    ...
    <main dir={fa ? "rtl" : "ltr"} className="bg-slate-50 py-8 text-slate-950 dark:bg-slate-950 dark:text-slate-50 small:py-14">
      <div className="content-container">
        <div className="mx-auto grid max-w-5xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_20px_60px_rgba(15,23,42,0.09)] dark:border-slate-800 dark:bg-slate-900 small:grid-cols-[42%_58%]">
          ...
          <section className="flex min-h-[560px] items-center p-6 xsmall:p-9 small:p-12"><div className="mx-auto w-full max-w-md">{children}<div className="mt-8 border-t border-slate-200 pt-6 dark:border-slate-800"><p className="text-sm leading-6 text-slate-500 dark:text-slate-400">{fa ? "متخصص هستید؟ پس از ساخت حساب معمولی، درخواست حساب حرفه‌ای ثبت کنید." : "Are you a professional? Create a regular account first, then submit a professional application."}</p><LocalizedClientLink href="/professional" className="mt-2 inline-block text-sm font-bold text-teal-700 hover:underline dark:text-teal-300">{fa ? "ثبت درخواست حرفه‌ای" : "Submit professional application"}</LocalizedClientLink></div></div></section>
        </div>
      </div>
    </main>
  )
